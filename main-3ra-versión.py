...
class EspacioVectorial(abc.ABC):
    ...
    def es_generador(self, conjunto_b: List[Any], dimension_espacio: int) -> bool:
        """
        Verifica si el conjunto B genera (span) el espacio vectorial.

        Un conjunto genera el espacio si y solo si el rango de la matriz
        de Gram G[i,j] = ⟨b_i, b_j⟩ es igual a la dimensión del espacio.

        Casos posibles:
          - rank(G) == dim  →  el conjunto genera el espacio ✅
          - rank(G) <  dim  →  faltan vectores para cubrir todas las direcciones ❌
        """
        n = len(conjunto_b)
        if n == 0:
            return False
        ...
        for i in range(n):
            for j in range(n):
                matriz_gram[i, j] = self.producto_interno(conjunto_b[i], conjunto_b[j])
        rango = np.linalg.matrix_rank(matriz_gram, tol=TOLERANCIA)
        return rango >= dimension_espacio
...
class EspacioPolinomios(EspacioVectorial):
    ...
    def __init__(self):
        self._dimension = None  # Se asigna al ingresar vectores

    def obtener_dimension(self, conjunto_b: List[np.ndarray]) -> int:
        if self._dimension is not None:
            return self._dimension
        # Fallback para base canónica (grado 2 → P_2, dimensión 3)
        if not conjunto_b:
            return 0
    ...
    def ingresar_vectores(self) -> List[np.ndarray]:
        ...
        if choice == "Usar base canónica.":
            self._dimension = 3  # Base canónica de P_2: {1, t, t²} → dimensión 3
            base_canonica = [np.array([1,0,0], dtype=float), np.array([0,1,0], dtype=float), np.array([0,0,1], dtype=float)]
            return base_canonica
        ...
        n = int(n_str)
        dim = n + 1
        self._dimension = dim  # Guardar la dimensión declarada por el usuario
...
class AnalizadorEspacioVectorial:
    ...
    def ejecutar_analisis(self, conjunto_b: List[Any]) -> None:
        ...
        es_gen = self.espacio.es_generador(conjunto_b, dimension)
        simbolo_gen = "✅" if es_gen else "❌"
        estado_gen = "ES GENERADOR" if es_gen else "NO ES GENERADOR"
        print(f"  CONJUNTO GENERADOR:    {simbolo_gen}  {estado_gen}")
        print(f"   (rank(G) {'≥' if es_gen else '<'} dim = {dimension})")
        if not es_gen:
            print("   (El conjunto no cubre todas las direcciones del espacio.)")
